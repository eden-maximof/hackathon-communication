...
while running:
    ...
    # we open here a new socket every time (loop)
    udp_client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP)  # UDP
    #udp_client_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1) # NOT WORKING ! WHAT IS IT?
    ...
    if tuple_info[0] == magic_cookie and tuple_info[1] == offer_message_type: # offer massege
        ...
        #Game mode
        # need to recive wellcame message here!!!
